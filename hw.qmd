...
The [Youtube video](https://www.youtube.com/watch?v=RGOj5yH7evk) that is listed in the [Project Management](https://statds.github.io/ids-f25/git.html) section of the class notes was a great introduction.
...
### Generating SSH key
 ...
 2. Now you can run the following command to generate the SSH key:
    ...
    + After this command, there will be few prompts:
      ...
      The .ssh folder is a hidden folder where all of your SSH keys will be stored.  You can toggle hidden files and folders by pressing Command + Shift + Period in Finder.  The "id_ed25519" is the default key name.
      If at a later time, you generate another key without specifying a name, you will be prompted whether or not you would like to overwrite the key.  There are instructions below on naming keys.
...
3. Now we can go over to the GitHub page to input our key.  You can give the key a name in the Title field and pasted your public key into the Key field.
...
There are two steps below, one for a generated key and the other if you named your key:
    ...
    It should be noted that the key you are using here is your private key, and do not include .pub.
2. In examples I have seen, this usually verifies the connection and everything is good to go.  However, I received the following message:
    ...
    You can visit [GitHub's SSH key fingerprints](https://docs.github.com/en/authentication/keeping-your-account-and-data-secure/githubs-ssh-key-fingerprints) to see their different public keys.
    From this website, you can copy their Ed25519 key and paste it into your terminal.  If it is a match, then everything should be good.  You may be asked to enter your passphrase if you are not running an SSH Agent.
...
The code in the following steps uses VSCode now we are setting up our computers to be able to use .qmd files in VSCode.
First, we must install Quarto on our computers, it can be found in this link: [Get Started](https://quarto.org/docs/get-started/).
You can choose your operating system follow the installation steps.
...
The code below uses VS Code.  There are a couple of steps before we can follow along with the instructions on then [Tutorial: Hello, Quarto](https://quarto.org/docs/get-started/hello/vscode.html) website.

### Install VS Code Quarto extension

This is another straightforward step where we can navigate to the **Extensions** tab in VS Code and search for Quarto.  Click the **Install** button after finding Quarto and VS Code should be all set.

### Setting up Python virtual environment

The code in the Quarto tutorial uses Python as well as specific packages.  It is best practice to set up a virtual environment and install any specific packages needed.  This allows for easier management of packages and dependencies, ensuring your code will run and not be affected by any updates outside of the isolated environment.  It also makes it easier to reproduce projects by having specific versions installed in the isolated environment.

1. In your terminal, navigate to your project's root directory.  I made the mistake of setting up my virtual environment in the project directory but it has to be set up in the directory above.  This way VS Code will be able to recognize it.  Now we can complete the following steps in the terminal:
...
2. Now we can install any specific packages we would like.  We are going to set up the [requirements.txt](https://github.com/statds/ids-f25/blob/main/requirements.txt) packages found in the class repository:
...
3. Next we will select the the virtual environment in VS Code.  Press Command + Shift + P to open the Command Palette.  Here you can type in **Python: Select Interpreter**.  You should see your virtual environment as one of the options, mine come up as *Python 3.13.7 ('.venv':venv)*.
...
4. Ask you to make the following small change to your code `theta = 4 * np.pi * r`.  This should results in the image below:
...
2. Navigate through the terminal to the directory where the .qmd homework assignment is located.
...
For this step, I initially opened the HTML in a browser and printed the PDF.  However, I did find steps to do this through the terminal:
...
After saving the PDF, click on **Create a new release** in the GitHub repository.  Add any titles or notes, and attach the PDF.  Click **Publish release**.